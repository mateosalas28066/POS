"""Pantalla de proveedores: tabla + formulario crear/editar. Lógica en ServicioProveedores."""
...
class PantallaProveedores(QWidget):
    def __init__(self, servicio: ServicioProveedores) -> None:
        super().__init__()
        self._servicio = servicio
        self._editando: Proveedor | None = None
    ...
    def al_mostrar(self) -> None:
        self._proveedores = self._servicio.listar()
        self._tabla.setRowCount(0)
        for p in self._proveedores:
            fila = self._tabla.rowCount()
            self._tabla.insertRow(fila)
            self._tabla.setItem(fila, 0, QTableWidgetItem(p.identificacion))
            self._tabla.setItem(fila, 1, QTableWidgetItem(p.nombre))
            self._tabla.setItem(fila, 2, QTableWidgetItem(p.contacto or ""))

    def _seleccionar_fila(self, fila: int) -> None:
        if not (0 <= fila < len(self._proveedores)):
            return
        p = self._proveedores[fila]
        self._editando = p
        self._identificacion.setText(p.identificacion)
        self._nombre.setText(p.nombre)
        self._contacto.setText(p.contacto or "")
        bloqueado = p.bloqueado_edicion
        ...
        self._estado.setText("Proveedor bloqueado" if bloqueado else "")

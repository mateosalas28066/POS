...
| NUBE2 | Plataforma web Fase 2: catálogo multi-local (maestro compartido + precio por local/overlay, importar por local, editar "aplicar a locales") + inventario multi-bodega (`ubicaciones` compartidas, movimientos entrada/salida/ajuste/traslado+confirmación/conversión) con **sync bidireccional híbrido** (catálogo snapshot, inventario delta append-only, LWW por `actualizado_en`), réplica RO en el POS editable por admin ([spec](superpowers/specs/2026-07-07-plataforma-web-fase-2-catalogo-inventario-design.md)) | 📋 planificado |
...
Suite: **419 passed** (`python -m pytest -q`, 2026-07-07).

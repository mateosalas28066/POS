...
| MARCA | Fundación visual del Sistema de Diseño "Carnes y Fruver RL" (solo tokens/tipografía/primitivos/referencia viva; sin rediseñar pantallas reales). **Web** (`pos-plataforma-web`): Tailwind v4 `@theme` de marca (dark-first, rojo `#E01E26`, glow neón) + fuentes auto-hospedadas Space Grotesk/Geist/Geist Mono, primitivos `Boton/Tarjeta/Campo/NavInferior` y página viva `#estilo` (gate `impeccable` limpio). **POS**: `qt-material` con tema `carnes_fruver.xml` + fuentes TTF vendorizadas (`carga_fuentes`), overlay `marca.qss` (roles `#primario/#rail/#kpi-valor/#card`, touch targets) + helper `aplica_glow`, y ventana `VentanaEstilo` (`python -m caja.pantalla_estilo`) ([spec](superpowers/specs/2026-07-08-sistema-diseno-marca-design.md) · [plan](superpowers/plans/2026-07-08-sistema-diseno-marca.md)) | ✅ implementado |
| WEBUI | Rediseño web: migración de las **pantallas reales** del frontend (`pos-plataforma-web`) al Sistema de Diseño de marca. Capa base de controles nativos + utilidades `.eyebrow`/`.kpi` (`index.css`); shell con header de marca y nav en **chip-tabs** (`App.tsx`); Login sobre `Tarjeta`/`Campo`/`Boton`; paleta de charts derivada de marca con rojo `#E01E26` racionado (barras a color, ya no negras; selección en rojo) validada con `dataviz`; Inventario y `BandejaPendientes`, Catálogo y `EditorOverlay` reescritos sobre los primitivos. Sin cambios de datos/auth/endpoints; barrido de clases/vars muertas de MARCA; gate por fase (`tsc -b` + `npm run build` + `impeccable` limpio) ([plan](superpowers/plans/2026-07-08-rediseno-web-slice-1.md)) | ✅ implementado (fase 1 + fase 2) |
| SHELL | Separación POS↔web — shell & UX base (Fase A): **POS** reenfocado al mostrador con rail de 4 vistas (Venta·Inventario·Reportes·Cierre), `CabeceraVista` (icono+título por vista), `BotonRail` con etiqueta bajo el icono y menú de perfil (cambiar contraseña / cerrar sesión) reemplazando el botón suelto de contraseña; **web** con `Sidebar` agrupada (Operación&análisis / Administración), `CabeceraVista` y `MenuPerfil`, vistas Reportes/Terceros/Despiece/Usuarios como placeholder para fases C/D/E. Solo UI en ambos repos ([plan](superpowers/plans/2026-07-08-separacion-pos-web-ux.md)) | ✅ implementado (Fase A) |
